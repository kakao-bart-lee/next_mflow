...
test.describe("온보딩 플로우", () => {
  ...
  test("온보딩 완료 시 메인 화면으로 전환된다", async ({ page }) => {
    // Step 0
    await page.locator("#birthdate").fill("1990-01-15")
    await page.locator("#birthtime").fill("14:30")
    await page.getByRole("button", { name: "다음" }).click()
    ...
    // 메인 앱 화면 (오늘의 운세 탭)
    await expect(page.getByText("오늘")).toBeVisible({ timeout: 10000 })
  })
  ...
  test("새로고침 후에도 온보딩 완료 상태가 유지된다", async ({ page }) => {
    ...
    await page.reload()
    // 온보딩 화면이 아닌 메인 앱 화면이 표시되어야 함
    await expect(page.getByText("사주 플레이북").first()).toBeVisible()
    await expect(page.getByText("언제 태어나셨나요?")).not.toBeVisible()
  })
})
