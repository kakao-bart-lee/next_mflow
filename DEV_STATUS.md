...
| 2026-03 | ADR-005 사용자 데이터 지속성 승인 (FortuneCache·PostgresStore·DailyCheckIn) | `9dc5b8a` |
| 2026-03 | 탐색 페이지 개편 + ChatInterface 통합 | `dfa3a98` |
| 2026-03 | targetDate API + auth 개선 | `c1790a4` |
...
| ADR | 제목 | 상태 | 구현 |
|-----|------|------|------|
| [ADR-001](./docs/adr/001-nextjs-proxy-convention.md) | Next.js 16 proxy 컨벤션 | Accepted | ✅ |
| [ADR-002](./docs/adr/002-mastra-memory-storage-binding.md) | Mastra Memory storage 연결 | Accepted | ✅ |
| [ADR-003](./docs/adr/003-mock-llm-structured-output.md) | MOCK_LLM structured output | Accepted | ✅ |
| [ADR-004](./docs/adr/004-terminology-washing-and-ux-layers.md) | 용어 워싱 + UX 레이어 | Accepted | ✅ |
| [ADR-005](./docs/adr/005-user-data-persistence.md) | 사용자 데이터 지속성 | Accepted | ✅ (완료) |

### ADR-005 구현 현황

| 항목 | 파일 | 상태 |
|------|------|------|
| FortuneCache 서비스 | `lib/services/fortune-cache.ts` | ✅ |
| PostgresStore 전환 | `lib/mastra/storage.ts` | ✅ |
| 채팅 Memory 에이전트 | `lib/mastra/agents/chat-agent.ts` | ✅ |
| 채팅 Thread API | `app/api/chat/threads/` | ✅ |
| DailyCheckIn/Action 모델 | `prisma/schema.prisma` | ✅ |
| 체크인/액션 API | `app/api/user/daily-*` | ✅ |
| SajuContext 클라이언트 캐시 | `lib/contexts/saju-context.tsx` | ✅ (사주 무한, 점성술 1일 TTL) |
...
| 항목 | 현황 | 필요 작업 |
|------|------|----------|
| **채팅 세션 지속성** | PostgresStore + Thread API 구현 완료 | SajuContext 캐시 연동 필요 |
| **구독 결제 플로우** | UI만 있음 | 결제 연동 없이 구독 로직 간소화 또는 수동 관리 |
| **월별 크레딧 자동 지급** | 미구현 | cron job 또는 serverless scheduled function |
| **사용자 프로필 이미지** | 미구현 | 업로드 UI + 저장 |
| ~~주간/대운 분석~~ | ✅ 완료 | LLM 동적 콘텐츠로 전환 완료 |
| **LLM 해석 결과 캐싱** | ✅ 구현 완료 | FortuneCache + interpret API 연동 완료 |
...
- **OAuth 실제 자격증명**: `SKIP_AUTH` 환경을 기본으로 사용하거나 Mock 인증 유지.
- **결제 연동 (Stripe/Portone)**: 실제 결제 시스템 도입 안 함.
- **이메일 발송 시스템 (SES/SendGrid)**: 실제 메일 발송 기능 제외.
